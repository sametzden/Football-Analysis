...
class TeamAssigner:
    ...
    def get_goalkeeper_team(self, frame, goalkeeper_bbox, goalkeeper_id, frame_width):
        """
        Assign a goalkeeper to a team based on their horizontal position on the pitch.
        Goalkeepers stay near their own goal throughout the match, so their x-position
        relative to the frame midpoint is a reliable team indicator.

        Convention:
          - Left half  (x_center < frame_width / 2)  → Team 1
          - Right half (x_center >= frame_width / 2) → Team 2
        """
